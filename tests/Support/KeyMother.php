...
final class KeyMother
{
    ...
    // Deliberate: derives rather than asserting the pair, so the fixture's public-key constant is checked against its private key instead of being taken on trust — see ADR-0061
    ...
    // Deliberate: the signer is stateless and memoised so each fixture call does not re-probe for libsecp256k1; the KeyPair itself is never cached because zero() mutates its private key — see ADR-0015
