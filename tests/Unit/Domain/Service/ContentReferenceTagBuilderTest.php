...
use Innis\Nostr\Core\Infrastructure\Service\Bech32EncoderAdapter;
...
final class ContentReferenceTagBuilderTest extends TestCase
{
    ...
    public function testBuildTagsExtractsTagsFromNostrReferences(): void
    {
        ...
        $pTags = array_filter($tagArrays, fn (array $t) => $t[0] === 'p');
        $eTags = array_filter($tagArrays, fn (array $t) => $t[0] === 'e');
        $qTags = array_filter($tagArrays, fn (array $t) => $t[0] === 'q');

        $pTagValues = array_map(fn (array $t) => $t[1], $pTags);
        $eTagValues = array_map(fn (array $t) => $t[1], $eTags);
        $qTagValues = array_map(fn (array $t) => $t[1], $qTags);
        ...
        $noteETag = array_values(array_filter($tagArrays, fn (array $t) => $t[0] === 'e' && $t[1] === $noteId));
        ...
        $neventTwoQTag = array_values(array_filter($tagArrays, fn (array $t) => $t[0] === 'q' && $t[1] === $neventTwoId));
