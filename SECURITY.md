...
- **NIP-04 legacy encrypted messages — shipped, deprecated, and unauthenticated.** `Nip04Cipher` implements NIP-04 exactly as specified: AES-256-CBC over the raw 32-byte ECDH shared secret, serialised as `base64(ciphertext)?iv=base64(iv)`. Both `Nip04EncryptionInterface` methods carry `#[Deprecated]`. It is documented here precisely because it is the weakest primitive in the package and the one a reader is most likely to assume is protected the way its NIP-44 sibling above is. It is not.

    - **There is no MAC.** Ciphertext is malleable — a hostile relay or peer can flip plaintext bits by flipping ciphertext bits and nothing detects it. The NIP-44 property stated above, MAC verified before unpadding, has no NIP-04 equivalent, because there is no MAC to verify.
    - **Decryption is a padding oracle, and this library cannot close it.** With no MAC, a tampered payload whose PKCS#7 padding happens to be valid *decrypts successfully* and returns garbage plaintext, while one with invalid padding throws. The distinguishing signal is therefore the outcome itself, which no choice of error text can hide, and the timing of an early framing rejection differs from that of a full block-cipher pass. `decrypt` does report one indistinguishable message for every rejection ([ADR 0058](docs/adr/0058-nip04cipher-reports-one-indistinguishable-decryption-failure.md)), which stops a consumer leaking *which stage* rejected a payload — but that narrows an information leak, it does not make the scheme authenticated. The oracle is a property of unauthenticated CBC and is closed only by not using NIP-04.
    - **Consumer responsibilities.** Do not expose NIP-04 decryption to untrusted input without rate limiting. Treat decrypt success-versus-failure as a sensitive signal — not a public API result, and not a per-attempt log line an attacker can observe. Use NIP-44 for anything new; NIP-04 is here for interoperability with existing kind-4 history, not because it is safe.
    - `decrypt` bounds the payload to 52–87472 bytes, sharing NIP-44's ceiling, so neither cipher can be made to buffer an unbounded frame.

- **NIP-49 password-encrypted private keys.** `Nip49Cipher` uses scrypt + XChaCha20-Poly1305, NFKC password normalisation per spec, and treats the password as a `Closure(): string` so the plaintext does not persist in caller scope. Spec vectors verified. Encryption floors the scrypt work factor at `logN 16` so the library never mints a brute-forceable `ncryptsec`, while decryption still accepts weaker values for interoperability; see [ADR 0030](docs/adr/0030-nip49-floors-encryption-logn-but-accepts-weaker-on-decrypt.md). Unlike secp256k1 and NIP-44, NIP-49 has **no pure-PHP fallback**: scrypt over raw `(N, r, p)` is reached only through `ext-ffi` + libsodium, so on a `gmp`-only host NIP-49 throws rather than degrading. The asymmetry with the FFI-optional signing path is deliberate; see [ADR 0039](docs/adr/0039-nip49-scrypt-requires-ffi-with-no-pure-php-fallback.md). Rejecting a tampered key-security byte rather than mapping it to `Unknown` is what makes the AEAD's associated-data authentication detect tampering; see [ADR 0009](docs/adr/0009-keysecuritybyte-frombyte-throws-on-unknown.md).

- **Private-key scalar validation.** `PrivateKey::tryFromHex` / `tryFromBytes` / `tryFromBech32` reject a scalar that is zero or `>= n` (the curve order), and `generate()` only ever mints one in `[1, n-1]`. This keeps the native and pure-PHP signing backends in agreement — the native path rejects an out-of-range scalar while the pure-PHP path would otherwise silently reduce it mod `n` and build a degenerate or aliased key; see [ADR 0029](docs/adr/0029-privatekey-rejects-scalars-outside-the-curve-order.md).
...
- **Defensive relay-protocol parsing.** `REQ` and `COUNT` messages cap filter count at 20; `Filter` and `TagFilter` cap each array-valued field at 1000 (the count of distinct tag names is not capped — see the relay-protocol parsing caps below); `SubscriptionId` restricts to printable ASCII; `OkMessage` requires a strict boolean for its `accepted` flag. Malformed relay input is rejected as a typed outcome — the message `tryFromArray` factories (and `JsonMessageDeserialiser` for JSON input) return `null` and the value-object constructors throw `InvalidArgumentException` — rather than propagating a `TypeError`.
...
- **Authenticity of payment events (NIP-57 zaps, NIP-61 nutzaps).** `ZapReceipt::tryFromEvent` and `Nutzap::tryFromEvent` are **parsers, not verifiers**. Neither checks a signature. Anyone can publish a kind 9735 or kind 9321 event, so nothing these types return is trustworthy until the consumer has verified the event itself — and displaying an unverified one as "X sent you N sats" is a spoofing vector, not a display bug.

    - **`ZapReceipt` does not authenticate anything — use `ZapReceiptVerifier` for that.** `getSenderPubkey()` is read from the `description` tag, an unverified JSON blob carried inside the receipt, and is attacker-chosen until verified. `ZapReceiptVerifier::verify($receipt, $lnurlProviderPubkey, $expectedLnurl)` applies NIP-57 Appendix F: the receipt's pubkey must equal the recipient's LNURL provider `nostrPubkey`, the BOLT-11 `invoiceAmount` must equal the zap request's `amount` tag, and the request's `lnurl` tag (when you supply an expected value) must match. It additionally verifies both the receipt's signature and the embedded zap request's own signature — the latter is not an Appendix F requirement, but without it all three conditions can hold while the provider attributes the zap to an arbitrary sender. See [ADR 0062](docs/adr/0062-zap-receipt-verification-takes-the-lnurl-provider-key-as-an-argument.md).
    - **You must supply the LNURL provider pubkey; it is the root of trust.** This package never fetches LNURL configuration, so it cannot discover that key — with the wrong one, a receipt from *any* provider verifies. Obtain it from the recipient's LNURL endpoint yourself. `ZapReceipt::tryFromEvent` also enforces one consistency rule on its own: the BOLT-11 amount is authoritative and it returns `null` when the request's `amount` tag disagrees, so a receipt cannot claim one figure in the invoice and another in the request — but that is a consistency check, not authentication.
    - **`Nutzap` does not validate Cashu proofs.** The proof amounts are parsed from tags; whether those proofs are genuine, unspent tokens is knowable only by redeeming them at the mint, which this package does not contact. Treat a parsed amount as a claim, not as funds. Its `getSenderPubkey()` is the event's own pubkey, which is authenticated **only if you verified the event signature first**.
    - **Verify before you parse.** Run `EventValidator::validateEvent()` (or `Event::verify()`) on any payment event before handing it to these types, then apply the NIP-57 LNURL-server check or the mint redemption yourself. Neither type performs those steps, and neither will fail if you skip them.

- **Safety of PHP's native `unserialize()` on library types.** Value objects in this library are not designed to round-trip through PHP's native `serialize()` / `unserialize()` functions. `unserialize` bypasses constructor validation and can instantiate library types in states the normal constructors would reject. Do not pass untrusted data through `unserialize()` into any library type. Parsing incoming relay JSON via `json_decode` and feeding the result into `Event::tryFromArray`, `Filter::tryFromArray`, or the message-layer factories is the intended flow, and those factories perform their own type validation.

- **Transport-layer limits.** `JsonMessageDeserialiser` decodes through `JsonWireFormat::decodeList`, which `json_validate`s then `json_decode`s at a nesting-depth limit of 512 and applies no input-size cap. WebSocket-frame size limits belong in the transport layer, outside this library.
...
Because the probe is silent — a host without the native library gets a working signer on the weaker path — both adapters expose `backend(): Secp256k1Backend`, reporting `Native` or `PurePhp` for the instance you hold rather than re-probing the host. It is deliberately declared on the concrete adapters and not on `SignatureServiceInterface` / `EcdhServiceInterface`, so a composition root can assert on it while no domain or application code can branch cryptographic behaviour on it. Asserting, reporting, and logging are the sanctioned uses. Full rationale: [ADR 0057](docs/adr/0057-secp256k1-adapters-report-their-active-backend-off-the-service-interfaces.md).
...
`Nip49EncryptionInterface::encrypt` and `::decrypt` take the password as a `Closure(): string` rather than a raw string. The adapter invokes the closure exactly once and `sodium_memzero`s both the revealed password and its NFKC-normalised copy on the way out. To make those wipes real rather than nominal, the revealed value is first detached into a buffer the adapter solely owns (`$revealed ^ str_repeat("\0", …)`): PHP strings are copy-on-write and `sodium_memzero` separates a shared string before writing, so wiping the provider's own return value would scrub a throwaway — the same trap [ADR 0028](docs/adr/0028-secretkeymaterial-expose-hands-a-detached-copy-so-the-wipe-is-effective.md) documents for `SecretKeyMaterial::expose`.

**This wipes the library's copy, not yours.** If your closure returns a variable you still hold — `static fn (): string => $password` — that binding is untouched and remains readable after the call; this is verifiable in a few lines and is not something the library can change, since the buffer is yours. The `Closure` shape pays off only when it *produces* the password without the caller retaining it: reading a prompt, unsealing it from a keystore, or decrypting it on demand. If you do keep the password in scope, zero it yourself. The alternative of accepting a raw `string` would be strictly worse — the value would then always be aliased in the caller's frame — but the closure is not a substitute for the caller managing its own secret.

### Strict 64-byte signatures

`Signature::tryFromHex` accepts only a complete 64-byte signature (exactly 128 lowercase hex characters) and returns `null` for anything shorter, longer, upper-case, or non-hex — it never zero-pads a short input, which could fabricate the wrong signature when the missing bytes came from `s`. If interoperability with leading-zero-stripping producers is ever required, it belongs in a verify-and-pick step above the value object. Full rationale: [ADR 0026](docs/adr/0026-signature-tryfromhex-requires-a-full-64-byte-signature.md).
...
`PrivateKey::tryFromBytes` and `ConversationKey::tryFromBytes` each allocate their own `SecretKeyMaterial` (via `SecretKeyMaterial::tryFromBytes`, which validates the length and returns `null` on a wrong size). A wrapping type therefore owns the buffer it zeroes: calling `zero()` on one `PrivateKey` cannot silently wipe the key material held by another object, because no API hands the same material instance to two wrappers. `SecretKeyMaterial`'s constructors (`__construct`, `tryFromBytes`, `tryFromHex`, `random`) are public; the lifecycle safety comes from each wrapper allocating a distinct material, not from hiding the factory.

### Gift-wrap outer signature verified, not merely present

`GiftWrapper::validateGiftWrap` cryptographically `verify()`s the outer wrap rather than trusting that it merely carries a signature. A gift-wrap `Event` always holds a well-formed signature by construction, but that guarantees only that a signature is present and correctly shaped — not that it is valid over the wrap's fields. NIP-44's AEAD MAC catches ciphertext tampering on its own, but a malicious relay or man-in-the-middle could still modify the outer wrap's non-content fields (tags, `created_at`, claimed pubkey) without the MAC firing. The outer `verify()` closes that gap.

### NIP-05 domain charset rejects IP literals

`Nip05Identifier::tryFromString` enforces a strict DNS-hostname pattern on the domain (lowercase alphanumerics, hyphens, dots, at least one label separator) and rejects IPv4 and IPv6 literals outright. It does not reject private-range public hostnames (for example `intranet.local`) because that decision is a policy concern that belongs in the HTTP adapter alongside the rest of SSRF mitigation. Splitting the concerns keeps the identifier type syntactically strict and the HTTP adapter semantically responsible.
...
`REQ` and `COUNT` cap filter count at 20 (`ReqMessage::MAX_FILTERS`, `CountMessage::MAX_FILTERS`). `Filter` caps each array-valued field at 1000 (`Filter::MAX_VALUES_PER_FIELD`), and `TagFilter` applies the same cap per tag name, on both the `fromValues` and `tryFromArray` paths. `SubscriptionId` is capped at 64 bytes of printable ASCII. The values match common relay defaults; consumers that want different limits should wrap the parsing layer rather than modify these constants.

**These are per-field bounds, not a bound on the frame.** The number of *distinct* tag names in a filter is deliberately not capped — NIP-01 places no limit on which single-letter or arbitrary tag names a client may query — so one `REQ` carrying 20 filters can still present many `#x` fields of 1000 values each. `TagFilter::matches()` iterates one index entry per tag name for every candidate event, so both the memory a filter pins and the per-event matching cost scale with a count the client chooses. Bounding total request size is the transport's job (see the transport-layer limits note above), and a relay that wants a ceiling on tag-name count or total filter values must impose it in the layer that owns its resource policy.
