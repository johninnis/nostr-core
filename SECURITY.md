...
- **Defensive relay-protocol parsing.** `REQ` and `COUNT` messages cap filter count at 20; `Filter` caps each array-valued field at 1000; `SubscriptionId` restricts to printable ASCII; `OkMessage` requires a strict boolean for its `accepted` flag. Malformed relay input is rejected as a typed outcome — the message `tryFromArray` factories (and `JsonMessageDeserialiser` for JSON input) return `null` and the value-object constructors throw `InvalidArgumentException` — rather than propagating a `TypeError`.
