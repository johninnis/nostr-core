...
- Several constants may hold the same wire value, because unrelated NIPs reuse the same letter for unrelated meanings: `MINT` and `URL` are both `'u'`, `PARENT_KIND` and `EXTERNAL_CONTENT_KIND` both `'k'`, `EXTERNAL_CONTENT` and `IDENTITY_CLAIM` both `'i'`. An enum could not express that at all — its cases are distinct by construction. Each name states what the tag means where it is used, so none is a duplicate to collapse onto another, and none is fenced at the code with a `// Deliberate:` marker: this consequence is the record for all of them.
