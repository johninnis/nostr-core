...
- **A parser of untrusted input** (`Event::tryFromArray` / `tryFromJson`, `Filter::tryFromArray`, `JsonMessageDeserialiser`, `Nip98Validator`) puts its failure in the return type — `?T`, or a sealed `*Failure` value — and must not throw.
