...
- Do not collapse a family onto one class with a `type` field, and do not turn `ReqMessage`/`CountMessage` into a single parameterised class — each removes a type that `MessageRouter` and `AmphpRelayConnection` match on with `instanceof`. (The *discriminant* is an enum; the *variants* are still distinct leaf classes.)
