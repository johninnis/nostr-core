# 22. `Rumour::tryFromArray` coerces non-string `content` to its JSON string rather than rejecting it
...
NIP-01 says an event's `content` is a string. `Rumour::tryFromArray` parses the untrusted unsigned-event fields from wire data (`Event::tryFromArray`/`tryFromJson` route through it), and every other field follows one rule: a type mismatch returns `null` and the whole event is rejected. `content` is the single exception — when it arrives as a non-string (a JSON object, number, boolean, or null, which `json_decode(..., true)` yields as a PHP array or scalar), `tryFromArray` does not reject the event. It re-encodes the value back to a JSON string and uses that as the content.
...
- A test pins the coercion (`testTryFromArrayHandlesNonStringContent`). Do not "tighten" `content` to reject non-strings for consistency with the other fields — the leniency is deliberate, and the safety argument above is why it does not compromise verification.
