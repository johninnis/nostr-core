...
ADR-0048 carries this record's inheritance sum-type decision forward unchanged and revises only the discriminant mechanism (the `protected const string TYPE` per leaf, surfaced through `getType(): string`) into a backed-enum `type()`. It is now the single, self-contained home for the message-hierarchy decision; read it for the current rationale. The record below is retained for history.
...
- An abstract `Message` base owns the one genuinely shared step — the decode in `tryFromJson()` and the `encode()` wire-flags helper — and declares the contract every message satisfies (`getType`, `toArray`, `tryFromArray`).
...
1. **The leaves are distinct nominal types matched at boundaries — but that alone would only need an interface.** `MessageDeserialiserInterface` returns `?ClientMessage`/`?RelayMessage`, so whatever reads a decoded message `instanceof`-matches the concrete leaf to reach its typed fields — a request handler branching over `EventMessage`/`ReqMessage`/`CloseMessage`/`AuthMessage`/`CountMessage`, a connection branching over `OkMessage`/`EoseMessage`/`ClosedMessage`/`NoticeMessage`/`AuthMessage`. Collapsing a family onto one class with a `type` string would turn those typed reads into runtime field-presence checks. Distinct types, though, are what an interface gives; they are not by themselves the reason for a base.
...
- Do not collapse a family onto one class with a `type` field, and do not turn `ReqMessage`/`CountMessage` into an enum or one parameterised class — each removes a type that a caller matches on with `instanceof`.
